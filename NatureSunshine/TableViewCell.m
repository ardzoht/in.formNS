...
- (NSArray *)UUChart_yValueArray:(UUChart *)chart{
    ...
    PFQuery *myQuery = [PFQuery queryWithClassName:@"Water"];
    [myQuery whereKey:@"username" equalTo:currentUser.username];
    [myQuery selectKeys:@[@"nOunces"]];
    ary = [myQuery findObjects];

    PFQuery *myQuery1 = [PFQuery queryWithClassName:@"Calories"];
    [myQuery1 whereKey:@"username" equalTo:currentUser.username];
    [myQuery1 selectKeys:@[@"nCalories"]];
    ary1 = [myQuery1 findObjects];
    
    PFQuery *myQuery2 = [PFQuery queryWithClassName:@"Weight"];
    [myQuery2 whereKey:@"username" equalTo:currentUser.username];
    [myQuery2 selectKeys:@[@"nPounds"]];
    ary2 = [myQuery2 findObjects];
    
    
    
    NSString *h;
    anAry = [[NSMutableArray alloc] init];
    anAry1 = [[NSMutableArray alloc] init];
    anAry2 = [[NSMutableArray alloc] init];
    

    for (int i = 0; i<ary.count; i++) {
        h = [NSString stringWithFormat:@"%d", [ary[i] intValue]];
        [anAry addObject:h];
    }
